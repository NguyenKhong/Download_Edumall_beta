...
def read_stream(stream, output, prebuffer, chunk_size=8192):
    ...
    show_player_progress = isinstance(output, PlayerOutput) and (output.namedpipe or output.filename or output.http) and sys.stdout.isatty()
    ...
    elif show_player_progress:
        stream_iterator = progress(stream_iterator,
                                   prefix=os.path.basename(args.player))
...
def setup_args(parser, options, config_files=[], ignore_unknown=False):
    """Parses arguments."""
    global args
    arglist = options
...
def setup_config_args(parser, options):
    config_files = []
    ...
    if config_files:
        setup_args(parser, options, config_files)
...
def main(options):
    ...
    setup_args(parser, options, ignore_unknown=True)
    ...
    setup_args(parser, options)
    setup_config_args(parser, options)
    ...
    if args.plugins:
        ...
        console.msg(msg)
    streamlink.http.close() # closing http connections
